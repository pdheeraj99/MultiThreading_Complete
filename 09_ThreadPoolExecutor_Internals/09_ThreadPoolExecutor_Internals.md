...
```mermaid
graph TD
    A(Start) --> B{Threads < corePoolSize?};
    B -- Yes --> C[Create Core Thread];
    B -- No --> D{Queue is full?};
    D -- No --> E[Add Task to Queue];
    D -- Yes --> F{Threads < maxPoolSize?};
    F -- Yes --> G[Create Surge Thread];
    F -- No --> H[Reject Task!];
    C --> Z(End);
    E --> Z;
    G --> Z;
...
This is a nightmare to manage with simple `Runnable`s and `Future.get()`. This is the problem of **asynchronous composition**. And the solution is one of the most powerful tools in modern Java: **`CompletableFuture`**. We will skip ahead to that in Chapter 11, but first, a classic pattern in Chapter 10. See you there! 🚀
